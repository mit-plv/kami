  ...
  Proof.
    ...
    - rewrite Ha.
      ...
      | [ H : (?x1, _) = (?x2, _) |- _ ] =>
        let Heq := fresh in
        let Heq' := fresh in
        assert (evalExpr (#(x1)!rv32iRq@."op")%kami_expr = evalExpr (#(x2)!rv32iRq@."op")%kami_expr) as Heq by (apply inv_pair in H; destruct H as [Heq' _]; rewrite Heq'; reflexivity);
          simpl in Heq
      end; subst.
      destruct x0; reflexivity.
  Qed.

/* 
 * matmul_init.c: four threads try to calculate the matrix multiplication in
 * parallel. In addition to the multiplication, this thread initializes the
 * target matrices. We test this multiplication by printing the trace of the
 * result matrix, which is done by [matmul_report.c]
 *
 * Expected output: 0
 */
...
int main()
{
  ...
  // initialization (only for tidx = 0)
  unsigned int a, b;
  ...
  // init done, time to calculate!
  *initd = 1;

  // init thread is also doing the partial matrix multiplication
  unsigned int i, j, k;

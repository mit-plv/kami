...
Require Import Ex.Timing.Specification Ex.Timing.Inversion Ex.Timing.ThreeStageInterface.
...
Module ThreeStageHiding (ThreeStage : ThreeStageInterface) (Hiding : ThreeStageModularHiding ThreeStage).
  ...
  Lemma write_read_consistent : forall lsp lsm,
      WellHiddenConcatSeq p m lsp lsm ->
  ...
  Proof.
    induction 1; intros;
      destruct lsp'; destruct lsm'; simpl in *; try congruence;
        repeat match goal with
               | [ H : context[_ :: _] |- _ ] => inv H
               end;
        eassert _ as whc_rq by (eapply whc_rq; eassumption);
        eassert _ as whc_rs by (eapply whc_rs; eassumption);
        match goal with
        | [ H : ?x ++ ?xs = ?y ++ ?ys |- _ ] => apply app_inv in H
        end;
        match goal with
        | [ |- context[length] ] => 
          erewrite <- censor_length_extract_wr by eassumption;
        ...
        f_equal; cbv zeta in *; shatter;
          try match goal with
              | [ H : (_ === calls ?l .[ rqMeth ])%fmap |- _ ] => 
                assert (_=== calls l .[ rsMeth ])%fmap by 
                    (eassert _ as e by (eapply pRqRs; eassumption); destruct e;
                     eassert _ as inv_rq_eq by (eapply inv_censoreq_rq_calls; try eassumption);
                     destruct inv_rq_eq;
                     eassert _ as inv_rs_eq by (apply inv_censoreq_rs_calls; eassumption);
                     destruct inv_rs_eq;
                     shatter;
                     try congruence;
                     try match goal with
          ...
          try match goal with
              | [ H : (_ === defs ?l .[ rqMeth ])%fmap |- _ ] => 
                assert (_=== defs l .[ rsMeth ])%fmap by 
          ...
          repeat rewrite rq_consistent in *;
          try match goal with
              |  [ Hs : ?x = Some _ , Hn : ?x = None |- _ ] => exfalso; rewrite Hn in Hs; discriminate
              end;
          try match goal with
              | [ _ : (_ === calls ?l .[ rsMeth ])%fmap, _ : (_=== defs ?l0 .[ rsMeth ])%fmap |- _ ] =>
                  exfalso; eassert _ as e by (eapply mRqRs; eassumption); destruct e;
                  eassert _ as inv_rq_eq by (eapply inv_censoreq_rq_memdefs; try eassumption);
                  destruct inv_rq_eq;
                  eassert _ as inv_rs_eq by (apply inv_censoreq_rs_memdefs; eassumption);
                  destruct inv_rs_eq;
              ...
              | [ _ : (_=== calls ?l .[ rsMeth ])%fmap, _ : (_ === defs ?l0 .[ rsMeth ])%fmap |- _ ] =>
                  ...
                  destruct inv_rs_eq;
                  shatter; try congruence
              | [ _ : (M.find rqMeth (defs _) = Some _), _ : (M.find rsMeth (defs _) = Some _) |- _ ] => 
                exfalso; eassert _ as e by (eapply mRqRs; eassumption); destruct e;
                  eassert _ as inv_rq_eq by (eapply inv_censoreq_rq_memdefs; try eassumption);
                  destruct inv_rq_eq;
                  eassert _ as inv_rs_eq by (apply inv_censoreq_rs_memdefs; eassumption);
          ...
          try match goal with
              | [ |- context[Defs.extractMethsRdVals] ] => unfold Defs.extractMethsRdVals;
                                                           repeat match goal with [ H : ?x = _ |- context[?x] ] => rewrite H end;
                                                           reflexivity + 
                                                           (subst lRq; destruct lastRq as [[[|] lastAdr]|]; simpl; try congruence;
                                                            simpl in H10; simpl in H3; shatter; congruence)
              end;
          try match goal with
              | [ H : (M.find rqMeth (defs ?l0)) = Some (existT SignT _ (?x, ?y)),
                      H' : (M.find rqMeth (defs ?l0)) = Some (existT SignT _ (?x', ?y')) |- _ ] => 
                try (assert (x = x') by reflexivity; fail 1);
                  rewrite H in H'; inv_rq_eq; assert (x = x') by (fin_func_tac; assumption);
                    subst; repeat match goal with [ H : ?z = ?z |- _ ] => clear H end
              end.
    ...
    - erewrite -> rqCall_from_censor in * by eassumption.
      ...
      eassert (getRqCall lRq l = _) as getCall by (unfold getRqCall;
                                                   repeat match goal with [ H : ?x = _ |- context[?x] ] => rewrite H end;
                                                   reflexivity).
      eassert (getRqDef lRq l0 = _) as getDef by (unfold getRqDef;
                                                  repeat match goal with [ H : ?x = _ |- context[?x] ] => rewrite H end;
                                                  reflexivity).
      repeat rewrite getCall in *; repeat rewrite getDef in *.
      match goal with
        [ H : Defs.ThreeStageMemMemConsistent _ ?x _ |- _ ] => specialize (IHWellHiddenConcatSeq x)
      end.
      simpl in IHWellHiddenConcatSeq. eapply IHWellHiddenConcatSeq; try eassumption.
      match goal with | [ H : Defs.ThreeStageMemMemConsistent ?x ?y ?z |- Defs.ThreeStageMemMemConsistent ?x ?y ?z' ] => 
                        replace z' with z; [apply H|]
      end.
      simpl in *. destruct (argV0 (Fin.FS Fin.F1)); congruence.
      

    - match goal with
      | [ H : (_ === defs ?l .[ rsMeth ])%fmap |- _ ] => 
        assert (_=== defs l .[ rqMeth ])%fmap by 
            (eassert _ as e by (eapply mRqRs; eassumption); destruct e;
             eassert _ as inv_rq_eq by (eapply inv_censoreq_rq_memdefs; try eassumption);
             destruct inv_rq_eq;
             eassert _ as inv_rs_eq by (apply inv_censoreq_rs_memdefs; eassumption);
      ...
      | [ H : (_ === calls ?l .[ rsMeth ])%fmap |- _ ] => 
        assert (_=== calls l .[ rqMeth ])%fmap by 
            (eassert _ as e by (eapply pRqRs; eassumption); destruct e;
             eassert _ as inv_rq_eq by (eapply inv_censoreq_rq_calls; try eassumption);
             destruct inv_rq_eq;
             eassert _ as inv_rs_eq by (apply inv_censoreq_rs_calls; eassumption);
      ...
      eassert (getRqDef lRq l0 = _) as getDef by (unfold getRqDef;
                                                  repeat match goal with [ H : ?x = _ |- context[?x] ] => rewrite H end;
                                                  reflexivity).
      repeat rewrite getCall in *; repeat rewrite getDef in *. subst.
      match goal with
        [ H : Defs.ThreeStageMemMemConsistent _ ?x _ |- _ ] => specialize (IHWellHiddenConcatSeq x)
      end.
      simpl in IHWellHiddenConcatSeq. eapply IHWellHiddenConcatSeq; try eassumption.
      ...
      destruct lastRq as [[[|] addr] |]; shatter; congruence.
    ...
    - erewrite -> rqCall_from_censor in * by eassumption.
      ...
      subst; simpl in IHWellHiddenConcatSeq. eapply IHWellHiddenConcatSeq; try eassumption.
...
Qed.     
  ...
  Proof.
    ...
    rewrite Hrp, Hrm in *.
    eassert (exists _, _) as MMCons by (eapply MemSpec; [eassumption|reflexivity]).
    destruct MMCons as [lastRq MMcons].
    assert (Defs.ThreeStageProcMemConsistent lsp lastRq mem) as Hpmc by (eapply ConcatMemoryConsistent; eauto). 
    ...
    | [ Hah : abstractHiding _ _ _ _,
      ...
      let Hph := fresh in
      pose (abstractToProcHiding lastRq _ _ _ _ Hah) as Hph;
        unfold Defs.ThreeStageProcHiding in Hph;
    ...
    - apply (forward_multistep_modular p m pequiv mequiv reginits defsDisj callsDisj validRegs); auto.
      ...
      + apply wellHidden_concat_modular_seq.
        ...
        eapply write_read_consistent; eauto.
    - subst.

...
Section Invariants.
  ...
  Definition p2st_scoreboard_waw_inv_body
  ...
  Record p2st_scoreboard_waw_inv (o: RegsT) : Prop :=
    { sbv0 : fullType type (SyntaxKind (Vector Bool rfIdx));
      ...
      Hinv0 : p2st_scoreboard_waw_inv_body d2efullv0 d2eeltv0 e2wfullv0 e2weltv0
                                           stallv0 stalledv0 sbv0 }.

  (* NOTE: this invariant requires p2st_scoreboard_waw_inv *)
  ...
  Record p2st_raw_inv (o: RegsT) : Prop :=
    { d2eeltv1 : fullType type (SyntaxKind d2eElt);
      ...
      Hd2einv1 : p2st_raw_inv_body d2efullv1 stallv1 d2eeltv1 stalledv1;
      He2winv1 : p2st_raw_inv_body e2wfullv1 stallv1 (evalExpr (e2wDecInst _ e2weltv1)) stalledv1;
      Hd2winv1 : p2st_raw_inv_body d2efullv1 e2wfullv1 d2eeltv1 (evalExpr (e2wDecInst _ e2weltv1))
    }.
  ...
  Record p2st_decode_inv (o: RegsT) : Prop :=
    { pgmv2 : fullType type (SyntaxKind (Vector (Data lgDataBytes) addrSize));
      ...
      Hd2einv2 : p2st_decode_inv_body pgmv2 rfv2 d2eeltv2 d2efullv2;
      He2winv2 : p2st_decode_inv_body pgmv2 rfv2 (evalExpr (e2wDecInst _ e2weltv2)) e2wfullv2
    }.
  ...
  Record p2st_stalled_inv (o: RegsT) : Prop :=
    { pgmv3 : fullType type (SyntaxKind (Vector (Data lgDataBytes) addrSize));
      ...
      Hinv3 : p2st_stalled_inv_body pgmv3 rfv3 stallv3 stalledv3 }.

  Definition p2st_exec_inv_body
  ...
  Record p2st_exec_inv (o: RegsT) : Prop :=
    { pcv4 : fullType type (SyntaxKind (Bit addrSize));
      ...
      Hinv4 : p2st_exec_inv_body pcv4 rfv4 e2wfullv4 e2weltv4 }.

  Definition p2st_epochs_inv_body
             (fepochv eepochv d2efullv e2wfullv e2dfullv stallv: fullType type (SyntaxKind Bool))
             (pcv: fullType type (SyntaxKind (Bit addrSize)))
             (d2eeltv: fullType type (SyntaxKind d2eElt))
             (e2weltv: fullType type (SyntaxKind e2wElt))
             (stalledv: fullType type (SyntaxKind d2eElt)) :=
    ...
    (* e2d and stalled *)
    (e2dfullv = true -> stallv = false) /\ (stallv = true -> e2dfullv = false) /\
    (fepochv = eepochv -> stallv = true ->
     ((d2efullv = true -> evalExpr (d2eEpoch _ d2eeltv) = fepochv) /\
      (e2wfullv = true -> evalExpr (d2eEpoch _ (evalExpr (e2wDecInst _ e2weltv))) = fepochv))) /\

    (* eepoch: e2d and {d2e, e2w} *)
  ...
  Record p2st_epochs_inv (o: RegsT) : Prop :=
    { pcv5 : fullType type (SyntaxKind (Bit addrSize));
      ...
      e2deltv5 : fullType type (SyntaxKind (e2dElt addrSize));
      He2deltv5 : M.find "e2d"--"elt"%string o = Some (existT _ _ e2deltv5);
      e2dfullv5 : fullType type (SyntaxKind Bool);
      He2dfullv5 : M.find "e2d"--"full"%string o = Some (existT _ _ e2dfullv5);
      ...
      Hinv5 : p2st_epochs_inv_body fepochv5 eepochv5 d2efullv5 e2wfullv5 e2dfullv5 stallv5
                                   pcv5 d2eeltv5 e2weltv5 stalledv5 }.

  Definition p2st_pc_inv_body
             (fepochv eepochv d2efullv e2wfullv e2dfullv stallv: fullType type (SyntaxKind Bool))
             (pcv: fullType type (SyntaxKind (Bit addrSize)))
             (d2eeltv: fullType type (SyntaxKind d2eElt))
             (e2weltv: fullType type (SyntaxKind e2wElt))
  ...
  Record p2st_pc_inv (o: RegsT) : Prop :=
    { pcv6 : fullType type (SyntaxKind (Bit addrSize));
      ...
      (* NOTE: Don't remove e2dElt even if it's not used in the invariant body. *)
      e2deltv6 : fullType type (SyntaxKind (e2dElt addrSize));
      He2deltv6 : M.find "e2d"--"elt"%string o = Some (existT _ _ e2deltv6);
      e2dfullv6 : fullType type (SyntaxKind Bool);
      He2dfullv6 : M.find "e2d"--"full"%string o = Some (existT _ _ e2dfullv6);
      ...
      Hinv6 : p2st_pc_inv_body fepochv6 eepochv6 d2efullv6 e2wfullv6 e2dfullv6 stallv6
                               pcv6 d2eeltv6 e2weltv6 stalledv6 }.

  Hint Unfold p2st_scoreboard_waw_inv_body p2st_raw_inv_body
       p2st_decode_inv_body p2st_stalled_inv_body
       p2st_exec_inv_body p2st_epochs_inv_body
       p2st_pc_inv_body : InvDefs.

  Ltac p2st_inv_old :=
    repeat match goal with
           | [H: p2st_scoreboard_waw_inv _ |- _] => destruct H
           | [H: p2st_raw_inv _ |- _] => destruct H
           | [H: p2st_decode_inv _ |- _] => destruct H
           | [H: p2st_stalled_inv _ |- _] => destruct H
           | [H: p2st_exec_inv _ |- _] => destruct H
           | [H: p2st_epochs_inv _ |- _] => destruct H
           | [H: p2st_pc_inv _ |- _] => destruct H
           end;
    kinv_red.
  ...
  Proof. (* SKIP_PROOF_ON
    ...
    - pose proof (p2st_epochs_inv_ok' H H0).
      kinvert.
      + mred.
      + mred.
      + kinv_dest_custom p2st_inv_tac.
      ...
      + kinv_dest_custom p2st_inv_tac; intuition idtac.
      + kinv_dest_custom p2st_inv_tac.
      + kinv_dest_custom p2st_inv_tac; rewrite H10 in H8; intuition idtac.
      + kinv_dest_custom p2st_inv_tac.
      + kinv_dest_custom p2st_inv_tac; rewrite H10 in H8; intuition idtac.
      + kinv_dest_custom p2st_inv_tac.
      + kinv_dest_custom p2st_inv_tac.
  ...
  Lemma p2st_scoreboard_waw_inv_ok':
    forall init n ll,
      init = initRegs (getRegInits p2stInl) ->
      Multistep p2stInl init n ll ->
      p2st_scoreboard_waw_inv n.
  Proof. (* SKIP_PROOF_ON
    induction 2.
    ...
    - kinvert.
      ...
      + mred.
      + kinv_dest_custom p2st_inv_tac.
      + kinv_dest_custom p2st_inv_tac; try (find_if_inside; intuition idtac; fail);
      ...
      + kinv_dest_custom p2st_inv_tac; try (rewrite e in H2; inv H2; fail);
  ...
  Proof. (* SKIP_PROOF_ON
    ...
    - pose proof (p2st_scoreboard_waw_inv_ok' H H0).
      kinvert.      
      + mred.
      + mred.
      + kinv_dest_custom p2st_inv_tac.
  ...
  Proof. (* SKIP_PROOF_ON
    ...
    - pose proof (p2st_raw_inv_ok' H H0).
      ...
      + kinv_dest_custom p2st_inv_tac; try reflexivity; try (rewrite e in H1; inv H1; fail).
      + kinv_dest_custom p2st_inv_tac; try reflexivity; try (rewrite e in H1; inv H1; fail).
      + kinv_dest_custom p2st_inv_tac; try (simpl; intuition idtac).
      + kinv_dest_custom p2st_inv_tac; try (simpl; intuition idtac).
      + kinv_dest_custom p2st_inv_tac.
      + kinv_dest_custom p2st_inv_tac.
      + kinv_dest_custom p2st_inv_tac.
  ...
  Lemma p2st_inv_ok:
    forall o,
      reachable o p2stInl ->
      p2st_scoreboard_waw_inv o /\ p2st_raw_inv o /\ p2st_decode_inv o /\
      p2st_stalled_inv o /\ p2st_exec_inv o /\ p2st_epochs_inv o /\ p2st_pc_inv o.
  Proof.
    intros; inv H; inv H0.
    repeat split.
    - eapply p2st_scoreboard_waw_inv_ok'; eauto.
    - eapply p2st_raw_inv_ok'; eauto.
    - eapply p2st_decode_inv_ok'; eauto.
    - eapply p2st_stalled_inv_ok'; eauto.
    - eapply p2st_exec_inv_ok'; eauto.
    - eapply p2st_epochs_inv_ok'; eauto.
    - eapply p2st_pc_inv_ok'; eauto.
  Qed.
...
Hint Unfold p2st_scoreboard_waw_inv_body p2st_raw_inv_body
     p2st_decode_inv_body p2st_stalled_inv_body
     p2st_exec_inv_body p2st_epochs_inv_body p2st_pc_inv_body : InvDefs.

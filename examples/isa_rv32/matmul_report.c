/* 
 * matmul_report.c: four threads try to calculate the matrix multiplication in
 * parallel. We test this multiplication by printing the trace of the result
 * matrix, which is done by this thread.
 *
 * Expected output: 16
 */
...
int main()
{
  ...
  unsigned int tidx = NUM_THREADS - 1;
  ...
  // waiting for all threads being finished
